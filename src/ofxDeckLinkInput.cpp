...
bool Input::start( BMDDisplayMode mode, bool use_rgb_colorspace )
{
    ...
    if( pDLInput->DoesSupportVideoMode( bmdVideoConnectionUnspecified, mode, pixelFormat, bmdSupportedVideoModeDefault, &result ) != S_OK )
    { return false; }
    ...
    if( pDLInput->GetDisplayMode( mode, &pDLDisplayMode ) != S_OK ) {
        ofLogError( "ofxDeckLinkAPI::Input" ) << "invalid display mode";
        return false;
    }
    ...
    fieldDominance = pDLDisplayMode->GetFieldDominance();
    ...
    if( pDL->QueryInterface( IID_IDeckLinkProfileAttributes, ( void** )&attr ) == S_OK ) {
...
bool Input::initDeckLink( int device_id )
{
    ...
    GetDeckLinkIterator( &pDLIterator );
...

    {
        ...
        HRESULT result = pDL->GetModelName( &deviceNameString );
        ...
        if( result == S_OK ) {
            ...
            device_name = deviceNameStream.str();
#endif
#ifdef TARGET_OSX
            char deviceName[64];
...
void Input::listDisplayMode()
{
    ...
    while( pDLDisplayModeIterator->Next( &pDLDisplayMode ) == S_OK ) {
        OFX_DECKLINK_COMSTR displayModeString = NULL;
        result = pDLDisplayMode->GetName( &displayModeString );
        ...
        if( result == S_OK ) {
            ...
            OFX_DECKLINK_COMSTR_RELEASE( displayModeString );
        }
...
HRESULT Input::VideoInputFormatChanged( BMDVideoInputFormatChangedEvents notificationEvents, IDeckLinkDisplayMode *newDisplayMode, BMDDetectedVideoInputFormatFlags detectedSignalFlags )
{
    ...
    // Pause video capture
    HRESULT result = pDLInput->PauseStreams();

    if( result == S_OK ) {
        // Enable video input with the properties of the new video stream
        result = pDLInput->EnableVideoInput( newDisplayMode->GetDisplayMode(), pixelFormat, bmdVideoInputEnableFormatDetection );
    }

    if( result == S_OK ) {
        // Flush any queued video frames
        result = pDLInput->FlushStreams();
    }

    if( result == S_OK ) {
        // Start video capture
        result = pDLInput->StartStreams();
    }
